...
Modelos receptores e modelos de dispersão são duas abordagens matemáticas
para determinar e quantificar o efeito das fontes poluidoras do ar. 
Os dois modelos podem se complementar e quando possível convém ao pesquisador usar as duas metodologias.%precisa avaliar melhor se você irá falar sobre modelos de dispersão - para quê e em que profundidade, para não ficar perdido no contexto do seu trabalho. Eventualmente você pode fortalecer apenas o uso de parâmetros meteorológicos como elemento que auxilia a análise dos resultados de modelos receptores.
%Não é verdade que modelos receptores prescindem do perfil das fontes. O Fator de Enriquecimento e CMB, primeiros destes modelos, somente são possíveis se você conhecer o perfil das fontes. PMF e AFP não necessitam deles para determinar "fatores principais" ou "principais fontes", mas que são genéricas e somente "adquirem vida" quando são identificadas pelo confronto com perfís conhecidos.
%fiz algumas sugestões, mas acho que você precisará cuidar melhor deste item.

No modelo de dispersão estima-se o campo de concentração de poluentes a 
partir de fontes com emissões conhecidas, combinado com dados meteorológicos (campo de ventos, altura da camada de mistura, turbulências etc). 

Modelos receptores buscam quantificar a participação de fontes poluidoras em amostras. O Balanço Químico de Massas (CBM), por exemplo, necessita do conhecimento prévio do perfil das fontes locais, cuja combinação linear proverá o rateio de amostragens entre tais fontes. Já a Análise de Fatores Principais (PFA) ou Positive Matrix Fatorization (PMF) permitem o rateio de fontes quando os seus perfis são desconhecidos. Entretanto, necessitamos de conhecê-los quando buscamos associar estes resultados a realidade concreta da bacia aérea estudada. 
...
%vai tratar de Análise Multivariada? cabe um subtítulo. 
...
Balanço Químico de Massa (BQM). %não consegui entender este parágrafo.
...
O Fator %de onde surgiu a palavra fator?
...
multivariadas são historicamente usadas.%parágrafo com redação e compreemsão confusos.
...
e outras informações que possam ajudar a fazer esse relacionamento devem ser usadas pelo pesquisador.
...
Os poluentes que são emitidos pelas fontes em uma bacia aérea devem chegar no receptor, podendo-se, assim, usar o princípio de conservação de massa no Modelo Receptor. 
...
%você deve terminar de dizer o que é o CMB antes de dizer suas condições de uso e limitações. Desse modo o leitor pode saber porque isso ocorre. Cabe avaliar como irá falar do CMB, já que não realizou nenhuma avaliação com ele.
...
% padronize a denominação dada à metodologia: Análise de Fatores Principais (AFP); Análise de Componentes Principais (ACP). Uma é geral. A outra é um método para resolvê-la.
...
e explicam a variação inicial do dados.%o que faz a componentes principais? Não está claro  no que disse. O que de diferente faz a AF? Quais são as equações básicas deste modêlo?
...
(citar equação).%Além de colocar a equação, antes de discutir o código numérico computacional,você deve discutir estatística e cientificamente o que faz o método. Apenas na sequência é que cabe discutir as facilidades oferecidas pelo programa computacional.
...
equação \ref{equation:pmf}.% veja que uij é a incerteza que temos na determinação da concentração (método analítico e amostragem); esse é um ponto importante e forte realizado neste trabalho e que vai precisar ser bemd iscutido, inclusive nesta parte teórica.
